...
final_state = jnp.concatenate([MW_state, LMC_state, particle_state], axis=0)
final_mass = jnp.concatenate([MW_mass, LMC_mass, particle_mass], axis=0) 
...
# Reverse simulation to check for numerical errors
# params_reverse = params._replace(t_end = -params.t_end)
# reversed_snapshots = time_integration(snapshots.states[-1], final_mass, config, params_reverse)
...
fig.add_trace(go.Scatter3d(
    ...
    mode='lines', showlegend=False, line=dict(
        color=jnp.arange(snapshots.states.shape[0]),  # time index
        colorscale='Viridis',
        width=4
    )
))
...
# # Track numerical errors by comparing the forward and backward integration
# fwd_times = np.array(snapshots.times)
# rev_times = np.array(-reversed_snapshots.times[::-1])

# # Interpolate reversed snapshots onto forward time grid
# # For each particle and each phase-space component
# reversed_states_interp = np.zeros_like(np.array(snapshots.states))

# for p in range(2):  # particles: MW, LMC
#     for d1 in range(2):  # pos, vel
#         for d2 in range(3):  # x, y, z
#             reversed_states_interp[:, p, d1, d2] = np.interp(
#                 fwd_times,
#                 rev_times,
#                 np.array(reversed_snapshots.states[::-1, p, d1, d2])
#             )

# # Now compare using absolute position error (avoids division by zero)
# pos_error_MW = np.linalg.norm(reversed_states_interp[:, 0, 0] - np.array(snapshots.states[:, 0, 0]), axis=-1) * length_scale
# pos_error_LMC = np.linalg.norm(reversed_states_interp[:, 1, 0] - np.array(snapshots.states[:, 1, 0]), axis=-1) * length_scale

# fig, axes = plt.subplots(1, 2, figsize=(14, 5))

# axes[0].plot(fwd_times * time_scale, pos_error_MW, label='MW')
# axes[0].plot(fwd_times * time_scale, pos_error_LMC, label='LMC')
# axes[0].set_xlabel("Time [Gyr]")
# axes[0].set_ylabel("Position error [kpc]")
# axes[0].set_title("Absolute position difference: forward vs backward")
# axes[0].legend()
# axes[0].grid()

# # Also plot relative to MW-LMC separation for context
# r_sep = np.linalg.norm(
#     np.array(snapshots.states[:, 1, 0]) - np.array(snapshots.states[:, 0, 0]), axis=-1
# ) * length_scale

# axes[1].plot(fwd_times * time_scale, pos_error_MW / r_sep * 100, label='MW')
# axes[1].plot(fwd_times * time_scale, pos_error_LMC / r_sep * 100, label='LMC')
# axes[1].axhline(5, color='red', linestyle='--', label='5%')
# axes[1].axhline(-5, color='red', linestyle='--')
# axes[1].set_xlabel("Time [Gyr]")
# axes[1].set_ylabel("Error / MW-LMC separation [%]")
# axes[1].set_title("Relative to MW-LMC separation")
# axes[1].legend()
# axes[1].grid()

# plt.tight_layout()
# plt.savefig("./notebooks/reflex_motion_forward_backward_diff.png", dpi=150)
