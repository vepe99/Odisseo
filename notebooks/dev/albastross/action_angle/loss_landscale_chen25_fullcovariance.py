...
if __name__ == "__main__":
    ...
    t_array = Quantity(-np.linspace(0, 3000, 200), "Myr")
    ...
    def loss(m_tot):
        pot_new = gp.CompositePotential(
            ...
            # disk = gp.MiyamotoNagaiPotential(m_tot=m_tot, a=3, b=0.28, units="galactic" ),
            # halo=gp.NFWPotential(m=m_tot, r_s=16, units="galactic"),
        ...
        t_array = Quantity(-jnp.linspace(0, 3000, 1000), "Myr")
        prog_mass = Quantity(10**4.5, "Msun")
        df = gd.ChenStreamDF()
        gen = gd.MockStreamGenerator(df, pot_new)
        stream_c25_new, _ = gen.run(jr.key(m_tot.astype(int)), t_array, w, prog_mass)
        ...
        log_marginal = marginalize_sigma_rho_grid(

            stream_likelihood_fullcov,
            stream, stream_target, noise_std,
            log_sigma_min=-1.0, log_sigma_max=2.0,
            n_sigma=100, rho_min=0.0, rho_max=0.3, n_rho=2
        )
    ...
    mass_NFW = jnp.sort(jnp.concatenate([jnp.linspace(true_NFW_mass * 0.25, true_NFW_mass * 2, 500), jnp.array([true_NFW_mass])]))
    mesh = Mesh(np.array(jax.devices()), ("i",))
    mass_NFW = jax.device_put(mass_NFW, NamedSharding(mesh, PartitionSpec("i")))
    # loss_value = jax.vmap(loss)(mass_NFW)
    loss_value = jax.lax.map(loss, mass_NFW, batch_size=12)
    ...
    # ax.set_xscale('log')
    # ax.set_yscale('log')
    plt.savefig('./plot/loss_landscape/loss_landscale_M_NFW_chen25_fullcovariance.png')