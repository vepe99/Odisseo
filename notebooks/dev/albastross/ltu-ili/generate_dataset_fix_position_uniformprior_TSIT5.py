...
def vmapped_run_simulation(rng_key, params_values):
    params_samples = SimulationParams(t_end = (3 * u.Gyr).to(code_units.code_time).value,  
                          Plummer_params = PlummerParams(Mtot=(10**4.05 * u.Msun).to(code_units.code_mass).value,
                                                        a=(8 * u.pc).to(code_units.code_length).value ),
                          NFW_params = NFWParams(Mvir=params_values[0] ,
                                               r_s= params_values[1]),
                          MN_params = MNParams(M = params_values[2] ,
                                              a = params_values[3] ,
                                              b = params_values[4] ),
                          PSP_params = PSPParams(M = params_values[5],
                                                alpha = 1.8, 
                                                r_c = params_values[6]),
                          G = code_units.G, )
...
for i in tqdm(range(name_str, num_chunks, batch_size)):
    ...
    parameter_value = jax.random.uniform(rng_key, 
                                         shape=(batch_size, 7), 
                                         minval=jnp.array([params_true.NFW_params.Mvir * code_units.code_mass.to(u.Msun) * 0.5,  
                                                           params_true.NFW_params.r_s * code_units.code_length.to(u.kpc) * 0.5, 
                                                           params_true.MN_params.M * code_units.code_mass.to(u.Msun) * 0.5, 
                                                           params_true.MN_params.a * code_units.code_length.to(u.kpc) * 0.5, 
                                                           params_true.MN_params.b * code_units.code_length.to(u.kpc) * 0.5,
                                                           params_true.PSP_params.M * code_units.code_mass.to(u.Msun) * 0.5,
                                                           params_true.PSP_params.r_c * code_units.code_length.to(u.kpc) * 0.5]), #vz

                                         maxval=jnp.array([params_true.NFW_params.Mvir * code_units.code_mass.to(u.Msun) * 2,
                                                           params_true.NFW_params.r_s * code_units.code_length.to(u.kpc) * 2,
                                                           params_true.MN_params.M * code_units.code_mass.to(u.Msun) * 2,
                                                           params_true.MN_params.a * code_units.code_length.to(u.kpc) * 2,
                                                           params_true.MN_params.b * code_units.code_length.to(u.kpc) * 2,
                                                           params_true.PSP_params.M * code_units.code_mass.to(u.Msun) * 2,
                                                           params_true.PSP_params.r_c * code_units.code_length.to(u.kpc) * 2,
                                         ])) #vz
    
    parameter_value_code_units = jnp.array([parameter_value[:, 0] * u.Msun.to(code_units.code_mass),
                                            parameter_value[:, 1] * u.kpc.to(code_units.code_length),
                                            parameter_value[:, 2] * u.Msun.to(code_units.code_mass),
                                            parameter_value[:, 3] * u.kpc.to(code_units.code_length),
                                            parameter_value[:, 4] * u.kpc.to(code_units.code_length),
                                            parameter_value[:, 5] * u.Msun.to(code_units.code_mass),
                                            parameter_value[:, 6] * u.kpc.to(code_units.code_length),]).T
    
    stream_samples = jax.vmap(vmapped_run_simulation, )(random.split(rng_key, batch_size)[:, 0], parameter_value_code_units)
    for j in range(batch_size):
        np.savez_compressed(f"/export/data/vgiusepp/odisseo_data/data_fix_position_uniform_prior_TSTIT5//file_{name_str:06d}.npz",
                            x = stream_samples[j],
