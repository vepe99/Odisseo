...
from odisseo.option_classes import SimulationConfig, SimulationParams, HernquistParams, NFWParams, MNParams, PSPParams, PointMassParams, DynamicalFrictionParams, PlummerParams
...
params = SimulationParams(t_end = 1 * u.Gyr.to(code_units.code_time),
                        ...
                        Plummer_params= PlummerParams(Mtot = 10**4.05 * u.Msun.to(code_units.code_mass),
...
vel_com = jnp.array([[109.5,-254.5,-90.3]]) * (u.km/u.s).to(code_units.code_velocity)
mass_com = jnp.array([params_com.Plummer_params.Mtot])
state_com = construct_initial_state(pos_com, vel_com) # state is a (N_particles x 2 x 3)

# Add the Milky Way as particle in index 0
MW_pos_com = jnp.array([[0., 0., 0.]]) * u.kpc.to(code_units.code_length)
MW_vel_com = jnp.array([[0., 0., 0.]]) * (u.km/u.s).to(code_units.code_velocity)
...
LMC_vel_com = jnp.array([[-63.9, -213.8, 206.6]]) * (u.km/u.s).to(code_units.code_velocity)
...
snapshots_com = time_integration(state_com, mass_com, config_com, params_com) # snapshots is a (N_snapshots x N_particles x 2 x 3)
...
sphere_pos, sphere_vel, sphere_mass = Plummer_sphere(key=key, params=params, config=config)
rel_sphere_pos = sphere_pos * u.kpc.to(code_units.code_length) + sphere_state_stream[0] 
rel_sphere_vel = sphere_vel * (u.km/u.s).to(code_units.code_velocity) + sphere_state_stream[1]
...
state_stream = jnp.concatenate([MW_state_stream, LMC_state_stream, particle_state_stream], axis=0)
...
fig.add_trace(go.Scatter3d(
    ...
    mode='markers', name='Plummer Sphere Start', marker=dict(color='green', size=2)
))
fig.add_trace(go.Scatter3d(
    x = snapshots.states[-1, 2:, 0, 0] * length_scale,
    y = snapshots.states[-1, 2:, 0, 1] * length_scale,
    z = snapshots.states[-1, 2:, 0, 2] * length_scale,
    mode='markers', name='Plummer Sphere End', marker=dict(color='red', size=2)
))

# MW trajectory
fig.add_trace(go.Scatter3d(
    x = snapshots_com.states[:, 0, 0, 0] * length_scale,
    y = snapshots_com.states[:, 0, 0, 1] * length_scale,
    z = snapshots_com.states[:, 0, 0, 2] * length_scale,
    mode='lines', showlegend=False, line=dict(
        color=snapshots_com.times * time_scale,  # time index
        colorscale='Viridis',
        reversescale=True,
...
fig.add_trace(go.Scatter3d(
    x = [snapshots_com.states[0, 0, 0, 0] * length_scale],
    y = [snapshots_com.states[0, 0, 0, 1] * length_scale],
    z = [snapshots_com.states[0, 0, 0, 2] * length_scale],
    mode='markers', name='MW', marker=dict(color='purple')
))
fig.add_trace(go.Scatter3d(
    x = [snapshots_com.states[-1, 0, 0, 0] * length_scale],
    y = [snapshots_com.states[-1, 0, 0, 1] * length_scale],
    z = [snapshots_com.states[-1, 0, 0, 2] * length_scale],
    mode='markers', showlegend=False, marker=dict(color='purple')
))

# LMC trajectory
fig.add_trace(go.Scatter3d(
    x = snapshots_com.states[:, 1, 0, 0] * length_scale,
    y = snapshots_com.states[:, 1, 0, 1] * length_scale,
    z = snapshots_com.states[:, 1, 0, 2] * length_scale,
    mode='lines', showlegend=False, line=dict(
        color=snapshots_com.times * time_scale,
        colorscale='Viridis',
        width=4
    )
))
fig.add_trace(go.Scatter3d(
    x = [snapshots_com.states[0, 1, 0, 0] * length_scale],
    y = [snapshots_com.states[0, 1, 0, 1] * length_scale],
    z = [snapshots_com.states[0, 1, 0, 2] * length_scale],
    mode='markers', name='LMC', marker=dict(color='blue')
))
fig.add_trace(go.Scatter3d(
    x = [snapshots_com.states[-1, 1, 0, 0] * length_scale],
    y = [snapshots_com.states[-1, 1, 0, 1] * length_scale],
    z = [snapshots_com.states[-1, 1, 0, 2] * length_scale],
...
fig.write_html("./notebooks/reflex_motion_tests/orbits_PlummerSphere.html")