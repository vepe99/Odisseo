...
- **JAX Native and Cross-Platform**: Built entirely on JAX, Odisseo enables end-to-end Just In Time (JIT) compilation for high performance (`jax.jit`), automatic vectorization (`jax.vmap`, or `jax.lax.map` for limited hardware resources) for trivial parallelization, and automatic differentiation (`jax.grad`). This ensures high performance across diverse hardware, including CPUs, GPUs, and TPUs. The use
